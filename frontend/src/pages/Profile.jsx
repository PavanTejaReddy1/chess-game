...
function StatCard({ emoji, label, value }) {
  return (
    <div className="bg-white/10 p-3 sm:p-4 rounded-xl text-center flex flex-col justify-center items-center gap-1">
      <p className="text-sm text-white/70">{emoji} {label}</p>
    ...
    <div className="min-h-[90vh] flex flex-col items-center justify-start text-white p-4 sm:p-6">
      ...
      <div className="w-full max-w-4xl bg-white/10 backdrop-blur-lg rounded-2xl p-4 sm:p-6 shadow-xl flex flex-col sm:flex-row items-center gap-4 sm:gap-6">
        ...
        <div className="flex-1 text-center sm:text-left">
          <h2 className="text-xl sm:text-2xl font-bold">{user?.name || "User Name"}</h2>
      ...
      <div className="w-full max-w-4xl mt-6 grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4">
        <StatCard emoji="⭐" label="Rating"       value={Math.floor(user?.stats?.rating) || 1200} />
        <StatCard emoji="♟️" label="Games Played" value={user?.stats?.gamesPlayed || 0} />
        <StatCard emoji="🏆" label="Wins"         value={user?.stats?.wins || 0} />
        <StatCard emoji="❌" label="Losses"       value={user?.stats?.losses || 0} />
      </div>

      {/* Stats row 2 */}
      <div className="w-full max-w-4xl mt-4 grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4">
        <StatCard emoji="🤝" label="Draws"          value={user?.stats?.draws || 0} />
        <StatCard emoji="🔥" label="Best Streak"    value={user?.stats?.bestStreak || 0} />
        <StatCard emoji="📈" label="Current Streak" value={user?.stats?.currentStreak || 0} />
      ...
      <div className="w-full max-w-4xl mt-6 grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="bg-white/10 p-4 rounded-xl">
          ...
          <div className="flex flex-wrap gap-4 text-center justify-center sm:justify-start">
            {[["🏅", "Champion"], ["⭐", "Unbeaten"], ["🔥", "First Win"], ["🎯", "10 Games"]].map(([icon, name]) => (
        ...
        <div className="bg-white/10 p-4 rounded-xl">
          ...
          <p className="text-sm text-white/50 italic">No recent matches yet</p>
        </div>
      </div>
      ...
      {editProfile && (
        <div className="fixed inset-0 flex items-center justify-center bg-black/50 backdrop-blur-sm z-50 px-4">
          <div className="bg-white/10 backdrop-blur-lg p-6 sm:p-8 rounded-2xl shadow-xl w-full max-w-[350px] relative">
            ...
            />
            <h2 className="text-xl sm:text-2xl font-bold mb-6 text-center">Upload Profile Picture</h2>
